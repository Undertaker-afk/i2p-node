...
export class I2PRouter extends EventEmitter {
  ...
  private setupNetDbListeners(): void {
    ...
    this.netDb.on('leaseSetStored', (evt) => {
    ...
    this.netDb.on('exploratoryLookup', ({ targetHash, floodfill }) => {
      ...
      this.sendExploratoryLookup(targetHash, floodfill).catch(() => {});
    });

    this.netDb.on('leaseSetLookup', ({ targetHash, floodfill }) => {
      this.sendLeaseSetLookup(targetHash, floodfill).catch(() => {});
    });
  }

  private setupNetDbRequestListeners(): void {
    this.netDbRequests.on('requestRouter', (hash) => {
      if (this.netDb.lookupRouterInfo(hash) || this.netDbRequests.hasRequest(hash)) {
        return;
      }
      const req = this.netDbRequests.createRequest(hash, false);
      if (req) {
        this.sendNextLookupRequest(req).catch(() => {});
      }
    });
    this.netDbRequests.on('sendNextRequest', (req) => {
      this.sendNextLookupRequest(req).catch(() => {});
    });
  }
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      // Bootstrap with zero-hop tunnels
  ...
  private async createRouterInfo(): Promise<void> {
    ...
    const addresses = [
      new RouterAddress('NTCP2', { host, port: ntcp2Port.toString(), v: '2' }, 5),
      new RouterAddress('SSU2', { host, port: ssu2Port.toString(), v: '2' }, 6),
    ];
    ...
    this.ntcp2PublishedIV = Crypto.randomBytes(16);

    const isPublished = host !== '0.0.0.0' && host !== '::' && ntcp2Port > 0;

    let ntcp2Opts = isPublished
      ? makeNtcp2PublishedOptions({
          host,
          port: ntcp2Port,
          staticKey: this.identity!.identity.encryptionPublicKey,
          ivB64: i2pBase64Encode(this.ntcp2PublishedIV),
          v: '2',
          caps,
        })
      : {
          ...
          v: '2',
          caps: caps.includes('4') || !caps.includes('6') ? '4' : '6',
        };

    this.wireRouterInfo = writeRouterInfoEd25519({
      identityBytes: this.identityEx!.identityBytes,
      publishedMs: Date.now(),
      addresses: [{ transportStyle: 'NTCP2', cost: isPublished ? 3 : 14, dateMs: 0, options: ntcp2Opts }],
      routerProperties: {
        netId: netId.toString(),
        caps,
        'router.version': '0.9.66',
  ...
  private async startTransports(): Promise<void> {
    this.ntcp2 = new NTCP2Transport({
      ...
      staticPrivateKey: this.identity!.encryptionPrivateKey,
      staticPublicKey: this.identity!.identity.encryptionPublicKey,
    ...
    this.ntcp2.on('error', (err) => {
      this.emit('error', { transport: 'NTCP2', error: err.error || err });
    });
  ...
  private async startSAM(): Promise<void> {
    this.sam = new SAMProtocol({ host: '127.0.0.1', port: this.options.samPort });
  ...
  private handleI2NPMessage(sessionId: string, message: any): void {
    logger.info(`Received I2NP message type=${message.type} from ${sessionId}`, undefined, 'Router');
    ...
    switch (message.type) {
      ...
      default:
        logger.debug(`Unhandled I2NP message type=${message.type}`, undefined, 'Router');
    }
  }

  private handleTunnelBuild(sessionId: string, message: any): void {
    logger.info(`TunnelBuild request received from ${sessionId} (not yet participating as hop)`, undefined, 'Router');
  ...
  private handleDatabaseStore(sessionId: string, message: any): void {
    ...
    logger.info(`DatabaseStore received type=${type} key=${key.toString('hex').slice(0, 16)}`, undefined, 'Router');

    let offset = 37;
    if (replyToken > 0 && buf.length >= offset + 36) {
      const replyTunnelId = buf.readUInt32BE(offset);
      offset += 4;
      const _replyGateway = buf.subarray(offset, offset + 32);
      offset += 32;

      if (replyToken !== 0xffffffff && this.ntcp2) {
        const dsWire = I2NPMessages.serializeMessage(I2NPMessages.createDeliveryStatus(replyToken, Date.now()));
        const targetSessionId =
          replyTunnelId === 0 && _replyGateway
            ? this.ntcp2.findSessionIdByRouterHash(_replyGateway) || sessionId
            : sessionId;
        this.ntcp2.send(targetSessionId, dsWire);
      }
    }
    ...
    if (type === 0 && data.length >= 2) {
      // RouterInfo
      try {
        const riLen = data.readUInt16BE(0);
        const ri = parseI2PRouterInfo(gunzipSync(data.subarray(2, 2 + riLen)));
        if (ri) {
          this.netDb.storeRouterInfo(ri);
        }
      } catch (err) {
        logger.debug(`Failed to parse RouterInfo from DatabaseStore: ${(err as Error).message}`, undefined, 'Router');
      }
    } else if (type === 1 || type === 3) {
      // LeaseSet1 or LeaseSet2
      const ls = type === 1 ? parseLeaseSetLS1(data, key) : parseLeaseSetLS2(data, key);
      if (ls) {
        logger.info(
          `Successfully parsed LeaseSet type=${type} for ${key.toString('hex').slice(0, 16)}`,
          undefined,
          'Router',
        );
        this.netDb.storeLeaseSet(ls);
        this.clearPendingLeaseSetRequest(key.toString('hex'));
      } else {
        logger.warn(`Failed to parse LeaseSet type=${type} for ${key.toString('hex').slice(0, 16)}`, undefined, 'Router');
      }
    }
  ...
  private handleDatabaseLookup(sessionId: string, message: any): void {
    ...
    const hasDelivery = (flags & 0x01) !== 0;
    const hasEcies = (flags & 0x10) !== 0;
    ...
    const numExcluded = buf.readUInt16BE(offset);
    offset += 2 + numExcluded * 32;

    let eciesSessionKey: Buffer | undefined;
    let eciesSessionTag: Buffer | undefined;

    if (hasEcies && buf.length >= offset + 41) {
      eciesSessionKey = buf.subarray(offset, offset + 32);
      offset += 33;
      eciesSessionTag = buf.subarray(offset, offset + 8);
    }

    const lookupType = (flags >> 2) & 0x03;
    let replied = false;
    ...
    if (lookupType === 0 || lookupType === 2) {
      ...
      const riWire =
        ri?.getWireFormatData() ||
        (this.routerInfo && key.equals(this.routerInfo.getRouterHash()) ? this.wireRouterInfo : null);

      if (riWire) {
        const c = gzipSync(riWire);
        const d = Buffer.concat([Buffer.alloc(2), c]);
        d.writeUInt16BE(c.length, 0);

        this.sendDatabaseLookupReply(
          sessionId,
          fromHash,
          replyTunnelId,
          I2NPMessages.createDatabaseStore(key, d, 0, this.routerInfo!.getRouterHash()),
          eciesSessionKey,
          eciesSessionTag,
        );
        replied = true;
    ...
    if (!replied && (lookupType === 0 || lookupType === 1)) {
      ...
      if (ls?.getWireFormatData()) {
        this.sendDatabaseLookupReply(
          sessionId,
          fromHash,
          replyTunnelId,
          I2NPMessages.createDatabaseStore(key, ls.getWireFormatData()!, 0, this.routerInfo!.getRouterHash(), ls.storeType),
          eciesSessionKey,
          eciesSessionTag,
        );
        replied = true;
    ...
    if (!replied && this.ntcp2) {
      const ffs = this.netDb.findClosestFloodfills(key, 3).map((f) => f.getRouterHash());
      this.sendDatabaseLookupReply(
        sessionId,
        fromHash,
        replyTunnelId,
        I2NPMessages.createDatabaseSearchReply(key, ffs, this.routerInfo!.getRouterHash()),
        eciesSessionKey,
        eciesSessionTag,
      );
    }
  ...
  private handleDatabaseSearchReply(sessionId: string, message: any): void {
    ...
    this.netDbRequests.handleSearchReply(
      parsed.key,
      parsed.routerHashes,
      this.netDbRequests.findRequest(parsed.key)?.isExploratory || false,
    );

    const keyHex = parsed.key.toString('hex');
    const req = this.pendingLeaseSetRequests.get(keyHex);

    if (req) {
      req.excluded.add(parsed.from.toString('hex'));
      for (const h of parsed.routerHashes) {
        const hHex = h.toString('hex');
        if (!req.excluded.has(hHex) && !req.candidateFloodfills.includes(hHex)) {
          req.candidateFloodfills.push(hHex);
        }
      }
      this.tryNextLeaseSetLookup(parsed.key);
    }

    this.emit('databaseSearchReply', { sessionId, message });
  }

  private handleDeliveryStatus(sessionId: string, message: any): void {
    logger.info(`DeliveryStatus received from ${sessionId}`, undefined, 'Router');
    this.emit('deliveryStatus', { sessionId, message });
  }

  private handleGarlic(sessionId: string, message: any): void {
    logger.info('Garlic message received', undefined, 'Router');

    const parsed = I2NPMessages.parseGarlicOuterMessage(message.payload);
    if (!parsed || parsed.body.length < 24) {
      return;
    }

    const possibleTag = parsed.body.subarray(0, 8);
    const tagHex = possibleTag.toString('hex');
    const pending = this.pendingEciesReplies.get(tagHex);

    let plaintext: Buffer | null = null;

    if (pending) {
      logger.info(`Attempting tag-based garlic decryption for tag=${tagHex}`, undefined, 'Router');
      try {
        plaintext = Crypto.decryptTaggedGarlicReply(pending.sessionKey, possibleTag, parsed.body.subarray(8));
      } catch (err) {
        logger.warn(`Tag-based garlic decryption failed: ${(err as Error).message}`, undefined, 'Router');
      }
    }

    if (!plaintext && this.identity && parsed.body.length >= 48) {
      logger.info('Attempting Noise_N garlic decryption', undefined, 'Router');
      try {
        plaintext = Crypto.decryptNoiseNGarlicReplyDirect(
          this.identity.encryptionPrivateKey,
          parsed.body.subarray(0, 32),
          parsed.body.subarray(32),
        );
      } catch (err) {
        logger.debug(`Noise_N garlic decryption failed: ${(err as Error).message}`, undefined, 'Router');
      }
    }

    if (!plaintext) {
      logger.info(`Garlic decryption failed for message from ${sessionId}`, undefined, 'Router');
      return;
    }

    logger.info(`Garlic decrypted successfully, tag=${tagHex}`, undefined, 'Router');
    ...
    logger.info(`Parsed ${cloves.length} cloves from garlic message`, undefined, 'Router');
  ...
  private async handleTunnelMessage(sessionId: string, message: any): Promise<void> {
    ...
    if (tunnel.hops.length === 0) {
      try {
        // Zero-hop inbound tunnel: data is the I2NP message itself with 4 bytes prefix
        if (data.length >= 8 && data[4] === 0x00) {
          this.handleI2NPMessage(sessionId, I2NPMessages.parseMessage(data.subarray(8)));
        }
      } catch (err) {
        logger.debug(`Failed to parse zero-hop tunnel message: ${(err as Error).message}`, undefined, 'Router');
      }
      return;
    }
    ...
    const { decryptHop } = await import('./tunnel/message.js');
    let decrypted = data;
    for (let i = tunnel.hops.length - 1; i >= 0; i--) {
      decrypted = decryptHop(decrypted, tunnel.hops[i].layerKey, tunnel.hops[i].ivKey);
    }

    const offset = decrypted.readUInt16BE(0);
    if (offset > 1021) {
      return;
    }
    ...
    try {
      ...
      logger.debug(`Failed to parse multi-hop tunnel message: ${(e as Error).message}`, undefined, 'Router');
    }
  }

  private handleTunnelBuildReply(sessionId: string, message: any): void {
    if (this.tunnelManager) {
      const records =
        message.type === I2NPMessageType.VARIABLE_TUNNEL_BUILD_REPLY
          ? I2NPMessages.parseVariableTunnelBuildReply(message.payload)
          : null;
      if (records) {
        this.tunnelManager.handleVariableTunnelBuildReply(message.uniqueId, records);
      }
    }
    this.emit('tunnelBuildReply', { sessionId, message });
  }

  private getNtcpEndpoint(ri: RouterInfo): { host: string; port: number } | null {
    const addr = ri.addresses.find(
      (a) =>
        a.transportStyle.toUpperCase().startsWith('NTCP') &&
        a.options.host &&
        !a.options.host.includes(':') &&
        a.options.s &&
        a.options.i &&
        a.options.port,
    );
    ...
    const host = addr.options.host.trim().replace(/^\[|\]$/g, '');
  ...
  private setupTunnelListeners(): void {
    ...
    this.tunnelManager.on('sendTunnelBuild', ({ firstHop, message }) => {
      this.sendI2NPDirect(firstHop, message).catch(() => {});
    });
  }
  ...
  private async sendExploratoryLookup(targetHash: Buffer, floodfill: RouterInfo): Promise<void> {
    const lookup = I2NPMessages.createDatabaseLookup(targetHash, this.routerInfo!.getRouterHash(), 3);
    await this.sendI2NPDirect(floodfill, lookup);
  }

  private async sendI2NPDirect(peer: RouterInfo, msg: any): Promise<void> {
    const wire = I2NPMessages.serializeMessage(msg);
    if (!this.ntcp2) {
      return;
    }

    const endpoint = this.getNtcpEndpoint(peer);
    if (endpoint) {
      try {
        await this.ntcp2.connect(endpoint.host, endpoint.port, peer);
        this.ntcp2.send(`${endpoint.host}:${endpoint.port}`, wire);

        this.stats.messagesSent++;
        this.stats.bytesSent += wire.length;

        logger.debug(`Successfully sent I2NP type=${msg.type} to ${endpoint.host}:${endpoint.port}`, undefined, 'Router');
      } catch (err) {
        logger.debug(
          `Failed to send direct I2NP to ${endpoint.host}:${endpoint.port}: ${(err as Error).message}`,
          undefined,
          'Router',
        );
      }
  ...
  private publishRouterInfo(): void {
    if (!this.routerInfo || !this.wireRouterInfo) {
      return;
    }

    const ffs = this.netDb.findClosestFloodfills(this.routerInfo.getRouterHash(), 3);
    const ourHash = this.routerInfo.getRouterHash();

    const c = gzipSync(this.wireRouterInfo);
    const data = Buffer.concat([Buffer.alloc(2), c]);
    data.writeUInt16BE(c.length, 0);

    const storeMsg = I2NPMessages.createDatabaseStore(ourHash, data, 0, ourHash);

    for (const ff of ffs) {
      this.sendI2NPDirect(ff, storeMsg).catch(() => {});
    }
  }
  ...
  async fetchLeaseSet(hash: Buffer, timeoutMs: number): Promise<LeaseSet | null> {
    ...
    if (!this.pendingLeaseSetRequests.has(targetHex)) {
      this.pendingLeaseSetRequests.set(targetHex, this.createPendingLeaseSetRequest(hash, targetHex, timeoutMs));
    }

    this.tryNextLeaseSetLookup(hash);

    return new Promise((resolve) => {
      const onStored = (evt: any) => {
        if (evt.hash.toString('hex') === targetHex) {
          cleanup();
          resolve(evt.leaseSet);
        }
      };
  ...
  private async sendLeaseSetLookup(targetHash: Buffer, floodfill: RouterInfo): Promise<void> {
    ...
    logger.info(
      `Sending LeaseSet lookup for ${keyHex.slice(0, 16)} to ${floodfill
        .getRouterHash()
        .toString('hex')
        .slice(0, 16)}`,
      undefined,
    ...
    const ffHash = floodfill.getRouterHash().toString('hex');
    if (req.excluded.has(ffHash)) {
      return;
    }

    req.excluded.add(ffHash);
    req.candidateFloodfills = req.candidateFloodfills.filter((c) => c !== ffHash);
    req.attempts++;
  ...
  private tryNextLeaseSetLookup(targetHash: Buffer): void {
    ...
    if (
      !req ||
      this.netDb.lookupLeaseSet(targetHash) ||
      Date.now() - req.createdAt >= LEASESET_REQUEST_TIMEOUT_MS ||
    ...
    while (req.candidateFloodfills.length > 0 && !next) {
      const cHash = req.candidateFloodfills.shift()!;
      if (!req.excluded.has(cHash)) {
        next = this.netDb.lookupRouterInfo(Buffer.from(cHash, 'hex')) || undefined;
      }
    }
    ...
    this.sendLeaseSetLookup(targetHash, next).catch(() => {});
  }
  ...
  private async sendDatabaseLookup(
    ...
    const opts: any = {};
    let fromHash = this.routerInfo!.getRouterHash();

    if (lookupType === 1) {
      const replyTunnel = await this.ensureLeaseSetReplyTunnel();
      if (!replyTunnel) {
        throw new Error('No reply tunnel');
      }

      fromHash = replyTunnel.gatewayHash;
      opts.replyTunnelId = replyTunnel.tunnelId;
      opts.eciesSessionKey = Crypto.randomBytes(32);
      opts.eciesSessionTag = Crypto.randomBytes(8);

      const tagHex = opts.eciesSessionTag.toString('hex');
      this.pendingEciesReplies.set(tagHex, {
        sessionKey: opts.eciesSessionKey as any,
    ...
    const wire = I2NPMessages.serializeMessage(
      I2NPMessages.createDatabaseLookup(targetHash, fromHash, lookupType as any, excludedPeers, opts),
    );

    const obTunnel = this.tunnelManager?.getOutboundTunnels().find((t) => t.hops.length > 0);

    if (obTunnel) {
      const payload = this.tunnelManager!.encryptForTunnel(obTunnel.id, wire)[0];
      const firstHop = obTunnel.hops[0];
      const ep = this.getNtcpEndpoint(firstHop.routerInfo);

      if (ep && this.ntcp2) {
        const wireMsg = I2NPMessages.serializeMessage({
        ...
        await this.ntcp2.connect(ep.host, ep.port, firstHop.routerInfo);
        this.ntcp2.send(`${ep.host}:${ep.port}`, wireMsg);

        this.stats.messagesSent++;
        this.stats.bytesSent += wireMsg.length;
      }
    } else {
      await this.sendI2NPDirect(floodfill, I2NPMessages.createDatabaseLookup(targetHash, fromHash, lookupType as any, excludedPeers, opts));
    }
  }
  ...
  private sendDatabaseLookupReply(
    ...
    let replyMsg = innerMsg;

    if (sessionKey && sessionTag) {
      const ciphertext = Crypto.encryptTaggedGarlicReply(
        sessionKey,
        sessionTag,
        I2NPMessages.createGarlicClovePayload([innerMsg]),
      );
      const body = Buffer.concat([sessionTag, ciphertext]);
      ...
      replyMsg = {
        ...
        uniqueId: Math.floor(Math.random() * 0xffffffff),
        expiration: Date.now() + 30000,
        payload: Buffer.concat([lenBuf, body]),
      };
    }

    const wire = I2NPMessages.serializeMessage(replyMsg);
    let finalWire = wire;

    if (replyTunnelId && replyTunnelId > 0) {
      const h = Buffer.alloc(4);
      h.writeUInt32BE(replyTunnelId, 0);
      finalWire = I2NPMessages.serializeMessage({
        type: I2NPMessageType.TUNNEL_GATEWAY,
        uniqueId: Math.floor(Math.random() * 0xffffffff),
        expiration: Date.now() + 30000,
        payload: Buffer.concat([h, wire]),
      });
    }

    this.ntcp2.send(this.ntcp2.findSessionIdByRouterHash(replyGatewayHash) || sessionId, finalWire as any);
    this.stats.messagesSent++;
    this.stats.bytesSent += finalWire.length;
  }
  ...
  private createPendingLeaseSetRequest(
    ...
    return {
      ...
      retryTimer: setTimeout(() => {
        this.tryNextLeaseSetLookup(targetHash);
      }, timeoutMs),
    };
  }
  ...
  private clearPendingLeaseSetRequest(targetHex: string): void {
    ...
    for (const t of req.eciesTags) {
      this.pendingEciesReplies.delete(t);
    }
  ...
  private cleanupPendingLeaseSetRequests(): void {
    for (const [t, r] of this.pendingLeaseSetRequests.entries()) {
      if (Date.now() - r.createdAt >= LEASESET_REQUEST_TIMEOUT_MS || r.attempts >= MAX_LEASESET_FLOODFILLS_PER_REQUEST) {
        this.clearPendingLeaseSetRequest(t);
      }
    }
  }
  ...
  private cleanupPendingEciesReplies(): void {
    const now = Date.now();
    for (const [t, p] of this.pendingEciesReplies.entries()) {
      if (now - p.createdAt >= ECIES_REPLY_TTL_MS) {
        this.clearPendingEciesReply(t);
      }
    }
  }

  private clearPendingEciesReply(tag: string): void {
    this.pendingEciesReplies.delete(tag);
    for (const r of this.pendingLeaseSetRequests.values()) {
      r.eciesTags.delete(tag);
    }
  }

  private async ensureLeaseSetReplyTunnel(): Promise<{ tunnelId: number; gatewayHash: Buffer } | null> {
    if (!this.tunnelManager || !this.routerInfo) {
      return null;
    }

    let t = this.tunnelManager.getInboundTunnels().find((c) => c.hops.length >= 0);
    if (!t) {
      t = (await this.tunnelManager.buildTunnel(TunnelType.INBOUND, 0)) ?? undefined;
    }

    return t
      ? {
          tunnelId: (t as any).hops[0]?.tunnelId ?? t.id,
          gatewayHash: (t as any).hops[0]?.routerHash ?? (this.routerInfo as any).getRouterHash(),
        }
      : null;
  }
}
