...
export class NTCP2Transport extends EventEmitter {
  ...
  private options: Required<Pick<NTCP2Options, 'host' | 'port' | 'netId'>> &
    Omit<NTCP2Options, 'host' | 'port' | 'netId'>;
  ...
  private async sendSessionRequest(sessionId: string): Promise<void> {
    const session = this.sessions.get(sessionId);
    if (!session || !session.remoteRouterHash || !session.remoteNtcp2IV || !session.remoteNtcp2Static)
      throw new Error('remote keys missing');
  ...
  private tryProcessSessionRequest(sessionId: string, session: NTCP2Session): boolean {
    ...
    if (
      !this.options.routerHash ||
      !this.options.publishedIV ||
      !this.options.staticPrivateKey ||
      !this.options.staticPublicKey
    )
      throw new Error('missing inbound keys');
    ...
    try {
      ...
      throw new Error('SessionRequest AEAD failed');
    }
    hs.h = sha256Concat(hs.h, frame);
    const netId = optsPlain.readUInt8(0),
      ver = optsPlain.readUInt8(1),
      padLen = optsPlain.readUInt16BE(2);
  ...
  private sendSessionCreated(sessionId: string, session: NTCP2Session): void {
    if (
      ...
      throw new Error('hs state missing');
    const hs = session.hs;
    const eph = Crypto.generateKeyPair();
    hs.rPriv = eph.privateKey;
    hs.rPub = eph.publicKey;
  ...
  private tryProcessSessionCreated(sessionId: string, session: NTCP2Session): boolean {
    ...
    hs.rPub = Buffer.from(y.subarray(0, 32));
    hs.h = sha256Concat(hs.h, hs.rPub);
    ...
    try {
      ...
      throw new Error('SessionCreated AEAD failed');
    }
    hs.h = sha256Concat(hs.h, frame);
  ...
  private tryProcessSessionConfirmed(sessionId: string, session: NTCP2Session): boolean {
    if (!session.hs || session.hs.m3p2Len === undefined || !this.options.staticPrivateKey || !session.hs.rPriv)
      throw new Error('hs state missing');
    const hs = session.hs,
      need = 48 + hs.m3p2Len!;
    if (session.recvBuffer.length < need) return false;
    const part1 = session.recvBuffer.subarray(0, 48),
      part2 = session.recvBuffer.subarray(48, 48 + hs.m3p2Len!);
    let sPub: Buffer;
    try {
      sPub = decryptWithAd(hs, part1, hs.h, 1);
    } catch (e) {
      throw new Error('SessionConfirmed part1 failed');
    }
    hs.h = sha256Concat(hs.h, part1);
    hs.remoteStatic = Buffer.from(sPub.subarray(0, 32));
    ...
    try {
      ...
      throw new Error('SessionConfirmed part2 failed');
    }
    hs.h = sha256Concat(hs.h, part2);
    const blocks = decodeBlocks(plain),
      riBlk = blocks.find((b) => b.type === 2);
  ...
  private tryProcessDataFrames(sessionId: string, session: NTCP2Session): boolean {
    ...
    const dp = session.dp,
      obf = session.recvBuffer.readUInt16BE(0),
      len = deobfuscateLength(dp.sipRecv, obf);
  ...
  private extractRemoteNtcp2Keys(ri: RouterInfo, hostHint?: string, portHint?: number): { s: Buffer; i: Buffer } {
    ...
    if (hostHint && typeof portHint === 'number') {
      ...
      const match = addrs.find((a) => a.options.host === hostHint && String(a.options.port) === portStr);
      if (match) addr = match;
    }
    const s = i2pBase64Decode(addr.options.s),
      i = i2pBase64Decode(addr.options.i);
  ...
  private sendDataFrame(session: NTCP2Session, plain: Buffer): void {
    ...
    const frame = encryptDataFrame(dp, plain),
      obfLen = obfuscateLength(dp.sipSend, frame.length),
...
function sha256Concat(h: Buffer, data: Buffer): Buffer {
  return Crypto.sha256(Buffer.concat([h, data]));
}
...
function mixKey(hs: NTCP2Handshake, ikm: Uint8Array): void {
  const temp = Crypto.hmacSHA256(hs.ck, ikm),
    ck = Crypto.hmacSHA256(temp, Buffer.from([0x01])),
    k = Crypto.hmacSHA256(temp, Buffer.concat([ck, Buffer.from([0x02])]));
  hs.ck = Buffer.from(ck);
  hs.k = Buffer.from(k);
  hs.nonce = 0;
}
...
function encryptWithAd(hs: NTCP2Handshake, plaintext: Buffer, ad: Buffer, nonce: number): Buffer {
  return Crypto.encryptChaCha20Poly1305(hs.k!, nonce12(nonce), plaintext, ad);
}
function decryptWithAd(hs: NTCP2Handshake, ciphertext: Buffer, ad: Buffer, nonce: number): Buffer {
  return Crypto.decryptChaCha20Poly1305(hs.k!, nonce12(nonce), ciphertext, ad);
}
function deriveDataPhase(ck: Buffer, h: Buffer, initiator: boolean): NTCP2DataPhase {
  const zerolen = Buffer.alloc(0),
    tempKey = Crypto.hmacSHA256(ck, zerolen);
  const k_ab = Crypto.hmacSHA256(tempKey, Buffer.from([0x01])),
    k_ba = Crypto.hmacSHA256(tempKey, Buffer.concat([k_ab, Buffer.from([0x02])]));
  const askMaster = Crypto.hmacSHA256(tempKey, Buffer.concat([Buffer.from('ask', 'ascii'), Buffer.from([0x01])]));
  const temp2 = Crypto.hmacSHA256(askMaster, Buffer.concat([h, Buffer.from('siphash', 'ascii')])),
    sipMaster = Crypto.hmacSHA256(temp2, Buffer.from([0x01]));
  const temp3 = Crypto.hmacSHA256(sipMaster, zerolen),
    sipkeys_ab = Crypto.hmacSHA256(temp3, Buffer.from([0x01])),
    sipkeys_ba = Crypto.hmacSHA256(temp3, Buffer.concat([sipkeys_ab, Buffer.from([0x02])]));
  const sip_ab = {
  ...
  return {
    sendKey: initiator ? k_ab : k_ba,
    recvKey: initiator ? k_ba : k_ab,
    sendNonce: 0,
    recvNonce: 0,
    sipSend: { ...(initiator ? sip_ab : sip_ba) } as any,
    sipRecv: { ...(initiator ? sip_ba : sip_ab) } as any,
  };
}
...
function obfuscateLength(sip: SipState, length: number): number {
  return (length ^ nextSipMask(sip)) & 0xffff;
}
function deobfuscateLength(sip: SipState, obf: number): number {
  return (obf ^ nextSipMask(sip)) & 0xffff;
}
function encryptDataFrame(dp: NTCP2DataPhase, plain: Buffer): Buffer {
  const ct = Crypto.encryptChaCha20Poly1305(dp.sendKey, nonce12(dp.sendNonce++), plain, Buffer.alloc(0));
  return ct;
}
function decryptDataFrame(dp: NTCP2DataPhase, frame: Buffer): Buffer {
  const pt = Crypto.decryptChaCha20Poly1305(dp.recvKey, nonce12(dp.recvNonce++), frame, Buffer.alloc(0));
  return pt;
}
